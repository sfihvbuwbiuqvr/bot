...
**کیفیت و صفحه‌بندی نتایج:**
- تا **۲۴ نتیجه** از هر جستجو گرفته می‌شود، **۷ تا در هر صفحه** نمایش داده می‌شود و دکمه‌های «◀️ قبلی / 📄 X از Y / بعدی ▶️» زیر لیست است.
- نتایج با **امتیاز relevance** مرتب می‌شوند: تطابق نام خواننده/آهنگ با عنوان و کانال (+)، برچسب Official/Lyrics (+)، طول منطقی آهنگ ۱:۲۰ تا ۸ دقیقه (+)؛ کاور/ریمیکس/بیکلام/اسلوند/تیزر/میکس طولانی/ست (−).
- **حذف تکراری:** هم بر اساس URL و هم بر اساس «عنوان+کانال» یکسان.
- دکمه‌ی **«همین را در پلتفرم مقابل هم بگرد»** پایین هر صفحه: همان کوئری را روی SoundCloud یا YouTube دوباره جستجو می‌کند.
...
## ۹.۲ 🛠 سوییچ سرویس‌ها و ابزارهای جدید پنل ادمین

با `/admin` → پنل مدیریت، ردیف‌های جدید:

| دکمه | کارکرد |
|---|---|
| **▶️ یوتیوب: 🟢/🔴** | با 🔴 کردن، هر درخواست یوتیوب (لینک، دانلود موزیک، پیش‌نمایش، رینگتون، جستجو، اینلاین، اسپاتیفای→MP3) پیام «🛠 سرویس یوتیوب فعلاً غیر فعال است!» می‌گیرد — برای همه‌ی کاربران از جمله ادمین (تا تست واقعی رفتار را ببینید). |
| **☁️ ساند‌کلود: 🟢/🔴** | همان منطق برای ساند‌کلود؛ دکمه‌های SoundCloud از منوی جستجو و زیر نتایج **غیب** می‌شوند. |
| **🔤 متن‌ترانه: 🟢/🔴** | خاموش‌کردن تشخیص آهنگ از متن ترانه (وقتی Genius/DDG بلاک‌اند و معطل نمی‌خواهی). |
| **🔎 اینلاین: 🟢/🔴** | خاموش‌کردن کامل `@BotName query`. |
| **🎙 شناسایی خودکار فایل: 🟢/🔴** | با 🔴 کردن، فرستادن ویس/آهنگ/ویدیو دیگر خودکار شناسایی نمی‌شود (پیش‌فرض 🟢 روشن). |
| **⏲ کول‌داون: 12s** | با فرستادن یک عدد (۰ تا ۳۰۰) فاصله‌ی درخواست‌های هر کاربر عوض می‌شود؛ ۰ = بدون محدودیت. |
| **🧹 پاکسازی فایل‌ها** | حذف فوری فایل‌های قدیمی‌تر از ۵ دقیقه در پوشه دانلود (وسط‌دانلود پاک نمی‌شود). |

**نکته:** وضعیت سوییچ‌ها در جدول `settings` دیتابیس ذخیره می‌شود و بعد از ری‌استارت ربات هم باقی می‌ماند. مقادیر: `yt_off`، `sc_off`، `lyrics_off`، `inline_off`، `auto_id_off`، `cooldown_sec`.

---

## ۹.۳ 🎙 شناسایی خودکار آهنگ از ویس/آهنگ/ویدیو

کافی است برای ربات **ویس (Voice)**، **فایل موسیقی (Audio/MP3)**، **ویدیو** یا **ریپلای ویدیویی** بفرستی؛ ربات خودکار:

1. فایل را از تلگرام دانلود می‌کند (سقف ۲۰ مگابایت — محدودیت API ربات).
2. با `ffmpeg` تکه‌ای صدا استخراج می‌کند.
3. با **Shazam** مقایسه و آهنگ را شناسایی می‌کند.
4. نتیجه را با همان دکمه‌های 🎧 دانلود موزیک / 📝 متن ترانه / ⏱ پیش‌نمایش / 🔔 رینگتون برمی‌گرداند.

- اگر فایل بیشتر از ۲۰ مگابایت باشد، پیام «تکه‌ی کوتاه‌تر بفرست یا لینک بده» نمایش داده می‌شود.
- اگر آهنگ شناسایی نشود، پیام «پیدا نشد» داده می‌شود (ویسِ خوانده‌شده/بوق‌بوق همیشه مچ نمی‌شود؛ کیفیت استودیویی بهتر مچ می‌شود).
- این قابلیت برای پست‌های **فورواردشده** هم کار می‌کند.
- با دکمه‌ی **🎙 شناسایی خودکار فایل** در پنل ادمین قابل خاموش/روشن‌کردن است.
- کپشنِ حاوی لینک، اولویت با دانلود لینک دارد (شناسایی انجام نمی‌شود).
